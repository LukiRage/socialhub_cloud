...
public class LoadDatabase {
    ...
    @Bean
    CommandLineRunner initUser(UserRepository userRepository, CommentRepository commentRepository, PostRepository postRepository, FollowingsRepository followingsRepository,GroupRepository groupRepository, DatabaseInitRepository databaseInitRepository) {
        return args -> {
            // Sprawdź czy baza została już zainicjalizowana
            DatabaseInit dbInit = databaseInitRepository.findById("INIT")
                    .orElse(new DatabaseInit());
            ...
            if (!dbInit.isInitialized()) {
                ...
                dbInit.setInitialized(true);
                databaseInitRepository.save(dbInit);
