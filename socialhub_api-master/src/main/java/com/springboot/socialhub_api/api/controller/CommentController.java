...
public class CommentController {
    ...
    public ResponseEntity<?> update(@RequestHeader("Authorization") String token, @RequestBody Comment updateComment) {
        if (authService.isLoggedIn(token)) {
            ...
            if (commentFromDatabase.isPresent()) {
                ...
                if (commentFromDatabase.get().getUser().getId() != currentUserId) {
    ...
    public ResponseEntity<?> delete(@RequestHeader("Authorization") String token, @PathVariable("comment_id") int id) {
        if (authService.isLoggedIn(token)) {
            Optional<Comment> commentFromDatabase = commentRepository.findById(id);
            if (commentFromDatabase.isPresent()) {
                int currentUserId = authService.getUserIdFromToken(token);
