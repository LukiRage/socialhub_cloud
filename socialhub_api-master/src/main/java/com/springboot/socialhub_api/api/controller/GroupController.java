...
public class GroupController {
    ...
    // Get all groups
    @GetMapping("/all")
    public ResponseEntity<List<Group>> getAllGroups(@RequestHeader(value = "Authorization", required = false) String token) {
        if (token == null || token.isEmpty() || !authService.isLoggedIn(token)) {
            return ResponseEntity.status(HttpStatus.UNAUTHORIZED)
                .body(new ArrayList<>()); // Return empty array instead of null
        }
    ...
    public ResponseEntity<Group> updateGroup(@RequestHeader("Authorization")String token,@PathVariable("group_id") int groupId,
        ...
        if(authService.isLoggedIn(token)){
            ...
            if (group.isPresent()) {
                ...
                if (existingGroup.getOwner_id() != currentUserId) {
    ...
    public ResponseEntity<Void> deleteGroup(@RequestHeader("Authorization")String token,@PathVariable("group_id") int groupId) {
        if(authService.isLoggedIn(token)){
            ...
            if (group.isPresent()) {
                ...
                // Check if current user is the owner
                if (group.get().getOwner_id() != currentUserId) {
                    return ResponseEntity.status(HttpStatus.FORBIDDEN).build();
                }
